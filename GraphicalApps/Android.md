...
2. Follow the site's instructions to "set up Android Studio on Mac OSX".
3. You may need to get the latest update. At the main screen, go to: "Configure", then "Check for Update".

## Create the ToDo project
...
Launch the emulator and let it run in the background. (green button under Actions)  
Android Studio's console should say *HAXM is working and emulator runs in fast virt mode*.

#### [Building the UI](http://developer.android.com/training/basics/firstapp/building-ui.html)
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(further reading: [App components, the essential building blocks of an Android app](http://developer.android.com/guide/components/fundamentals.html))  
...
Finally, implement and add the appropriate listeners for the button click and the list item's long click. (don't forget to add the onClick property for each button)
